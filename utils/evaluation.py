...
def NDCG_based_on_single_sample(sampled_ranking,q_doc_weights,rank_weights,cutoff):
    ...
    return NDCG
def NDCG_based_on_datasplit(sampled_ranking,qid,dataSplit,rank_weights,cutoff):
    """
    ...
    return NDCG
def Update_NDCG_multipleCutoffs(sampled_ranking,qid,dataSplit,rank_weights,cutoffs,NDCGDict):
    """
    ...
    for cutoff in cutoffs:
        ...
        NDCGCur=NDCG_based_on_datasplit(sampled_ranking,qid,dataSplit,rank_weights,cutoff)
...
def outputNDCG(NDCGDict,OutputDict):
    ...
    for key,value in NDCGDict.items():
        OutputDict[key+"_cumu"]=dicounted_metrics(value)[-1]
        OutputDict[key+"_aver"]=np.mean(value)
...
def outputFairness(data,OutputDict):
    """
