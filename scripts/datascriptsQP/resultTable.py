...
metric_name=['NDCG_1_aver','NDCG_3_aver','NDCG_5_aver',"disparity","time1kLists"]
...
for positionBiasSeverity in positionBiasSeverities:
    ...
    for metrics in metric_name:
        for datasets,data_name_cur in data_rename.items():
            ...
            result_validated["LP"]=result["fairness_strategy_LP"]["n_futureSession_1"]["fairness_tradeoff_param_1000"]["exploration_tradeoff_param_0.0"]
            ...
            result_validated["QPFair"]=result["fairness_strategy_QPfairNDCG"]["n_futureSession_200"]['fairness_tradeoff_param_1.0']
    ...
    output_path=os.path.join(OutputPath,positionBiasSeverity+"mean_latexQP.csv")
    r.to_csv(output_path)
    output_path=os.path.join(OutputPath,positionBiasSeverity+"mean_std_latexQP.csv")
    rstd.to_csv(output_path)
    mean=results_org.to_mean(result_dfram)
    output_path=os.path.join(OutputPath,positionBiasSeverity+"meanQP.csv")
