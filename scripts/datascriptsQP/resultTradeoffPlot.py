...
for positionBiasSeverity in positionBiasSeverities:
    ...
    for datasets,data_name_cur in data_rename.items():
        ...
        result_validated["QPFair (Ours)"]=result["fairness_strategy_QPfairNDCG"]["n_futureSession_200"]
        result_validated["QPFair-Horiz"]=result["fairness_strategy_QPfairNDCGHorizontal"]["n_futureSession_200"] 

        # result_validated["GradFair"]=results_org.getGrandchildNode(result["fairness_strategy_FairCo_average"],"exploration_tradeoff_param_10")
        ...
        for ind,metrics in enumerate(metric_name):
            ...
            fig.savefig(os.path.join(OutputPath,metric_name_dict[metrics[1]]+positionBiasSeverity+data_name_cur+"tradeoffplotQP.pdf"), dpi=600, bbox_inches = 'tight', pad_inches = 0.05)
            plt.close(fig)