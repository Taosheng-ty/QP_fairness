...
for positionBiasSeverity in positionBiasSeverities:
    ...
    for datasets,data_name_cur in data_rename.items():
        ...
        # result_validated["LP_200"]=result["fairness_strategy_LP"]["n_futureSession_200"]
        ...
        # result_validated["QPfairNDCG_200"]=results_org.getGrandchildNode(result["fairness_strategy_QPfairNDCG"]["n_futureSession_200"],"exploration_tradeoff_param_0")
        # result_validated["QPfairNDCG_200Hori"]=results_org.getGrandchildNode(result["fairness_strategy_QPfairNDCGHorizontal"]["n_futureSession_200"],"exploration_tradeoff_param_0")
        result_validated["QPFair (Ours)"]=results_org.getGrandchildNode(result["fairness_strategy_QPfairNDCG"]["n_futureSession_200"],"exploration_tradeoff_param_5")
        result_validated["QPFair-Horiz"]=results_org.getGrandchildNode(result["fairness_strategy_QPfairNDCGHorizontal"]["n_futureSession_200"],"exploration_tradeoff_param_5")
        ...
        for ind,metrics in enumerate(metric_name):
            ...
            legend=results_org.reorderLegend(config.desiredQPFair,axs) 
            fig.savefig(os.path.join(OutputPath,metric_name_dict[metrics[1]]+positionBiasSeverity+data_name_cur+"tradeoffplotQPOnline.pdf"), dpi=600, bbox_inches = 'tight', pad_inches = 0.05)
            plt.close(fig)