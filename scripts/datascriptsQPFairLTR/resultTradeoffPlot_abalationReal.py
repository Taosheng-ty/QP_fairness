...
for positionBiasSeverity in positionBiasSeverities:
    ...
    for datasets,data_name_cur in data_rename.items():
        ...
        result_validated["Eff.+Fair.+Uncert."]= results_org.getGrandchildNode(result["fairness_strategy_GradFair"],"exploration_tradeoff_param_20")
        ...
        for ind,metrics in enumerate(metric_name):
            ...
            axs.legend()
            fig.savefig(os.path.join(OutputPath,metric_name_dict[metrics[1]]+positionBiasSeverity+data_name_cur+"Abalation_tradeoffplot.pdf"), dpi=600, bbox_inches = 'tight', pad_inches = 0.05)
            plt.close(fig)